            ...
            <div class="col-md-12 mb-1">
                    ...
                    <div class="card border-0">
                        ...
                        <div class="col-md-1 mb-2 d-flex ">
                            ...
                            <img src="{{ asset('storage/vendor/products/images/' . $product->image) }}" class=""
                        ...
                        <div class="card-footer py-0 border-0">
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="btn-group">
                                    ...
                                    <form action="{{ route('deleteProduct',['id' => $store->id, 'product_id' => $product->id]) }}" method="get">
                                        @csrf
                                        <a href="{{ route('editProductPage', ['vendorName' => $vendor->name, 'id' => $product->id]) }}" class="btn btn-sm btn-outline-secondary me-2 py-0">Edit</a>
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-outline-danger py-0"
                                            onclick="return confirm('Are you sure you want to delete this product?')">Delete</button>
                                    </form>
                                </div>
                                <small class="text-muted">Last updated: {{ $product->updated_at->format('d/m/Y')
                                    }}</small>
                            </div>
                        </div>
                    </div>
