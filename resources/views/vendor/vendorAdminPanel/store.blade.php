            ...
            <div class="row">
                <div class="col">
                    <nav aria-label="nav navbar" class="bg-light rounded-3 p-3 mb-4">
                        <ol class="nav mb-0">
                            <li class="nav-item me-4"><a class="btn text-primary"
                                    href="{{ route('editStorePage', ['vendorName' => $vendor->name]) }}"><i
                                        class="fas fa-pencil-alt me-1"></i>Edit Store</a></li>
                            <li class="nav-item me-4"><a class="btn text-primary"
                                    href="{{ route('storePreview', ['id' => $store->id]) }}"><i
                                        class="fas fa-expand me-1"></i>Full Screen Preview</a></li>
                            <li class="nav-item me-4">
                                {{-- <a class="btn text-danger"
                                    href="{{ route('deleteStore', ['id' => $store->id]) }}"><i
                                        class="fas fa-trash-alt me-1"></i>Delete</a> --}}

                                <form action="{{ route('deleteStore', ['id' => $store->id]) }}" method="get">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn text-danger"
                                        onclick="return confirm('Are you sure you want to delete your Store?')"><i
